...
<dl>
  ...
  <dd>
    <ul>
      <li>setup.shスクリプトのNTPサーバ設定のデフォルト値に不備(3台とも同じホスト名になっていた）があったため修正しました。</li>
      <li>「会社と自宅でIPのセグメントが重なった場合」の説明に一部抜けがあったので追記しました。</li>
    </ul>
  </dd>
  <dt>2020/04/27</dt>
  <dd>
    <ul>
      <li>通信回線が不安定な時にRemminaがVPNセッションまで切断すると使いにくいため、設定を変更してマニュアルに説明を追加ました。</li>
      <li>セットアップマニュアルのキーボードレイアウトの変更手順を修正しました。</li>
    </ul>
  </dd>
</dl>
...
* 間に挟むルータの、自宅ネットワークに接続する(WAN)側に、固定IP(例: 192.168.0.251)を設定する
* Raspberry Pi に接続する(LAN)側でDHCPサービスを有効にして、192.168.1.0/24などの別セグメントのIPアドレスを割り当てる<br>
  (ルータとRaspberry Piの両方で固定IPとルーティングの設定ができればそれでも可）
* 自宅の上流のルータで、192.168.1.0/24のルーティングを間に挟んだルータのWAN側アドレス(192.168.0.251)に設定する
